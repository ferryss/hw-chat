...
if __name__ == '__main__':
    import argparse
    from hwrag.config import Config
    ...
    config = {
        ...
        # 以下是必须结合自己实际情况修改的参数
        'dataset_path': '/root/autodl-tmp/hw-chat/hw-chat-0.3/hwrag/data/test',   # 评估数据集的存放路径
        "index_path": "/root/autodl-tmp/hw-chat/hw-chat-0.3/knowledge_base/wiki/vector_store/bge-large-zh-v1.5/index.faiss",  # 向量索引存放路径
        "corpus_path": "/root/autodl-tmp/hw-chat/hw-chat-0.3/hwrag/data/indexes/sample_data.jsonl",  # 原始文本存放路径
        "retrieval_model_path": model_config.MODEL_PATH["embed_model"]["bge-large-zh-v1.5"],  # 用于检索的模型存放路径
        "retrieval_cache_path": "/root/autodl-tmp/hw-chat/hw-chat-0.3/hwrag/data/cache",    # 检索缓存存放目录
        "generator_model": "chatglm3-6b",  # 生成模型名称
        "generator_model_path": model_config.MODEL_PATH["local_model"]["chatglm3-6b"],  # 生成模型存放路径
        "save_dir": "/root/autodl-tmp/hw-chat/hw-chat-0.3/hwrag/data/result",    #评估结果存放路径
        ...
        # "use_reranker": False,
        "use_reranker": True,
        "rerank_model_path": model_config.MODEL_PATH["reranker"]["bge-reranker-large"],
        "rerank_model_name": model_config.RERANKER_MODEL,
        "rerank_topk": 3,
