...
async def chat(query: str = Body(..., description="用户输入", examples=["你好"]),
    ...
    async def chat_iterator() -> AsyncIterable[str]:
        ...
        if history:  # 优先使用前端传入的历史消息
            # 使用前端传来的历史消息，需要使用包含history的prompt
            prompt = get_prompt_template("llm_chat", "with_history")
            if prompt is None:
                prompt = get_prompt_template("llm_chat", "default")
            chat_prompt = PromptTemplate.from_template(prompt)
            # 构建历史消息字符串
            history_str = "\n".join([f"{h.role}: {h.content}" for h in history])
            memory = None  # 前端已提供history，不需要memory
            ...
            history_str = None  # memory 会自动处理
        else:
            # 无历史消息，使用默认prompt
            prompt = get_prompt_template("llm_chat", "default")
            if prompt is None:
                raise ValueError("Prompt template not found for llm_chat/default")
            chat_prompt = PromptTemplate.from_template(prompt)
            memory = None
            history_str = None
        ...
        if history_str is not None:
            chain_inputs["history"] = history_str

        # Begin a task that runs in the background.
        task = asyncio.create_task(wrap_done(
            chain.acall(chain_inputs),
            callback.done),
        )
...
if __name__ == '__main__':
    ...
    # openai.base_url = "http://127.0.0.1:20000/v1/"
    # 
    # model = "zhipu-api"
